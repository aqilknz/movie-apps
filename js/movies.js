...
// VALIDASI HALAMAN & SESSION
const session = JSON.parse(localStorage.getItem('currentUser'));
if (!session) {
    window.location.href = 'login.html';
} else {
    const userNameDisplay = document.getElementById('userNameDisplay');
    if (userNameDisplay) userNameDisplay.innerText = session.email;
}
...
function renderMovies(movies) {
    ...
    movies.forEach(movie => {
        ...
        const movieCard = `
            <article class="flex flex-col md:flex-row gap-6 bg-zinc-800 p-5 rounded-2xl border border-zinc-800 hover:border-pink-600/50 transition-all duration-300 group">
                <div class="relative overflow-hidden rounded-xl shrink-0">
                    <img src="${movie.poster_path ? IMG_URL + movie.poster_path : 'https://via.placeholder.com/160x240?text=No+Image'}" 
                         class="w-full md:w-40 h-64 md:h-60 object-cover transform group-hover:scale-120 transition duration-500">
                </div>
